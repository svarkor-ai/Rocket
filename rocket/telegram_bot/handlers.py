...
async def om_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        "• ADX — trendstyrka (icke-riktad)\\n\\n"
        "〰️ *Volatilitet* (vikt: 20%)\\n"
        "• Bollinger Bands — prisposition i bands\\n"
        "• ATR — genomsnittlig prisrörelse (volatilitet, ej riktning)\\n"
        "• Donchian Channel — position i högsta/lägsta kanal\\n\\n"
        "📊 *Volym* (vikt: 10%)\\n"
        "• OBV — On-Balance Volume, kumulativ volymflöde\\n"
        "• MFI — Money Flow Index (volymvägd RSI)\\n"
        "• VWAP — Volume-Weighted Average Price\\n\\n"
        "☁️ *Avancerade* (ingår i trend)\\n"
        "• Ichimoku Cloud — helhetssystem (trend+stöd+motstånd)\\n"
        ...
        "signaländring för dina prenumerationer."
    )
...
async def scanall_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    chat_id = update.effective_user.id
    admin_chat_id = int(os.environ.get("SCAN_PRO_ADMIN_CHAT_ID", "0"))  # 0 = nobody (MC#939)
